...
class SearXNGSearchProvider(BaseSearchProvider):
    ...
    def _extract_public_instances(cls, payload: Any) -> List[str]:
        ...
        for raw_url, item in instances.items():
            ...
            if not isinstance(uptime, (int, float)) or float(uptime) <= 0:
                continue
            ranked.append((float(uptime), cls._search_latency_seconds(item), raw_url.rstrip("/")))
        ...
        with cls._public_instances_lock:
            ...
            try:
                ...
                if response.status_code != 200:
                    logger.warning("[SearXNG] 拉取公共实例列表失败: HTTP %s", response.status_code)
                else:
            ...
            if stale_urls:
                ...
                logger.warning(
                    ...
                    len(stale_urls), cls.PUBLIC_INSTANCES_STALE_REFRESH_BACKOFF_SECONDS,
                )
                return stale_urls
            cls._public_instances_stale_retry_after = now + cls.PUBLIC_INSTANCES_STALE_REFRESH_BACKOFF_SECONDS
            logger.warning(
    ...
    def search(self, query: str, max_results: int = 5, days: int = 7) -> SearchResponse:
        ...
        if self._base_urls:
            candidates = self._rotate_candidates(self._base_urls, max_attempts=len(self._base_urls))
            retry_enabled = True
        ...
        for base_url in candidates:
            response = self._do_search(query, base_url, max_results, days=days, timeout=timeout, retry_enabled=retry_enabled)
